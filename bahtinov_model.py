...
def pad_data(data, bp_factor, scaling_factor, factor = 4):
...
def slice_bandpass():
    ...
    for Q in quadrants:
        for f in tqdm(filters_name):
            ...
            np.savetxt('/media/data/bahtinov_results/Model/Data/Filter_%s_quadrant%s_sliced.txt'  %(f, Q), bandpass_sliced)
            #fig = figure()
...
def create_bandpass_defocus(delta):
    ...
    filters_name = ['u', 'g', 'q', 'r', 'i', 'z']
    mask_resized = cv2.reseize(mask_data, (int(scaling_factor * mirror), int(scaling_factor * mirror)))

    for f in tqdm(filters_name):
        bandpass = np.loadtxt('/media/data/bahtinov_results/Model/Data/Filters/Filter_%s_quadrant%s.txt' %(filters_name[m], Q))
